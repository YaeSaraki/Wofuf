...
import dev.saraki.wofuf.modules.forum.infra.repos.jpa.mappers.CommentDetailsMapper
...
class CommentRepoImpl(
    ...
    override fun findCommentDetailsByPostSlug(
        postSlug: PostSlug,
        memberId: MemberId?,
        offset: Int?
    ): List<CommentDetails> {
        val commentEntities = commentJpaRepo.findByPostEntity_Slug(postSlug.value)

        return commentEntities.map {
            commentEntity ->
                CommentDetailsMapper.toDomain(commentEntity)
        }.subList(offset ?: 0, commentEntities.size)
    }

    override fun findCommentDetailsByCommentId(commentId: CommentId): CommentDetails? {
        val commentEntity = commentJpaRepo.findById(commentId.stringValue).orElse(null)
