...
async function fetchHotPosts() {
  ...
  const result = await forumService.getPopularPosts(5)
  if (result.isSuccess) {
    hotPosts.value = result.getValue().posts.slice(0, 5)
  }
  ...
  <aside class="bf-sidebar">
    ...
    <div class="bf-sidebar__section">
      ...
      <ul v-else-if="hotPosts.length > 0" class="bf-hot-list">
        ...
        >
          <span class="bf-hot-rank" :class="{ 'bf-hot-rank--top': index < 3 }">{{ index + 1 }}</span>
