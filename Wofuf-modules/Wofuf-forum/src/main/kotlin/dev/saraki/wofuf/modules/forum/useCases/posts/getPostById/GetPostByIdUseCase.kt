 ...
 * @author YaeSaraki
 * @date 2026/4/16
 * @description Use case for getting a post by ID
 */
@Service
...
) : UseCase<GetPostByIdDto.Request, GetPostByIdDto.Response> {

    override fun execute(request: GetPostByIdDto.Request): Result<GetPostByIdDto.Response> {
        // 1. Validate post ID
        if (request.postId.isBlank()) {
            return GetPostByIdErrors.PostIdEmptyError()
        }

        // 2. Create PostId from string
        val postIdOrError = try {
            PostId.create(UniqueEntityId(request.postId))
        } catch (e: IllegalArgumentException) {
            return GetPostByIdErrors.InvalidPostIdError(request.postId)
        }
        ...
        val post = postRepo.findPostByPostId(postId)
            ?: return GetPostByIdErrors.PostNotFoundError(request.postId)
        ...
        val member = memberRepo.findMemberById(post.memberId)
            ?: return GetPostByIdErrors.MemberNotFoundError(request.postId)
        ...
        return Result.success(GetPostByIdDto.Response(postDto))
    }
